...
wget --retry-connrefused --read-timeout=20 --timeout=15 --tries=10 --continue --no-check-certificate https://repo.jellyfin.org/releases/server/windows/stable/combined/jellyfin_10.8.13.zip
7z x "jellyfin_10.8.13.zip" -o%CD%
mkdir "%PROGRAMFILES%\Jellyfin"
mkdir "%PROGRAMFILES%\Jellyfin\Server"
xcopy /s /i /y "jellyfin_10.8.13" "%PROGRAMFILES%\Jellyfin\Server"

cd %~dp0
rmdir /s /q "%~dp0jellyfin_10.8.13"
del /s "%~dp0jellyfin_10.8.13.zip"
